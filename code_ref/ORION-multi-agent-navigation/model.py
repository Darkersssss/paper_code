...
class PolicyNet(nn.Module):
    def __init__(self, node_dim, embedding_dim):
        ...
        # update graph encoder
        self.update_graph_encoder = Encoder(embedding_dim=embedding_dim, n_head=8, n_layer=6)

        # cross attention to the features
        self.feature_combine = Decoder(embedding_dim=embedding_dim, n_head=8, n_layer=1)
        ...
        # option-critic modules
        self.termination_head = nn.Sequential(
            nn.Linear(embedding_dim, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 1) # add sigmoid activation later
        )
        self.option_selector = nn.Sequential(
            nn.Linear(embedding_dim, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, 2)  # logits over FSM states (2 options)
        )
        self.option_embedding1 = nn.Embedding(2, embedding_dim)
        self.option_embedding2 = nn.Embedding(2, embedding_dim)  # 2 options

    def encode_graph(self, node_inputs, node_padding_mask, edge_mask, navi_edge_mask, navi_node_inputs):
        node_feature = self.initial_embedding(node_inputs)
        navi_node_feature = self.initial_embedding(navi_node_inputs)
        enhanced_node_feature = self.encoder(src=node_feature,
                                             key_padding_mask=node_padding_mask,
                                             attn_mask=edge_mask)
        enhanced_navi_feature = self.update_graph_encoder(src=navi_node_feature,
                                                          key_padding_mask=node_padding_mask,
                                                          attn_mask=navi_edge_mask)

        # do the cross attention
        enhanced_graph_feature, _ = self.feature_combine(enhanced_node_feature, enhanced_navi_feature, node_padding_mask)

        return enhanced_graph_feature

    def decode_state(self, enhanced_graph_feature, current_index, node_padding_mask,
                     current_option):
        embedding_dim = enhanced_graph_feature.size()[2]
        current_node_feature = torch.gather(enhanced_graph_feature, 1,
                                            current_index.repeat(1, 1, embedding_dim))
        current_node_feature = current_node_feature + self.option_embedding2(current_option).squeeze(1)  # (batch_size, 1, embedding_dim)
        enhanced_current_node_feature, _ = self.decoder(current_node_feature,
                                                        enhanced_graph_feature,
                                                        node_padding_mask)
    ...
    def output_policy(self, current_node_feature, enhanced_current_node_feature,
                      enhanced_graph_feature, current_edge, edge_padding_mask):
        embedding_dim = enhanced_graph_feature.size()[2]
        ...
        neighboring_feature = torch.gather(enhanced_graph_feature, 1,
    ...
    def forward(self, node_inputs, node_padding_mask, edge_mask, current_index,
                ...
                prev_option, check = False):
        enhanced_graph_feature = self.encode_graph(node_inputs, node_padding_mask, edge_mask, navi_edge_mask, navi_node_inputs)

        embedding_dim = enhanced_graph_feature.size()[2]
        current_node_feature = torch.gather(enhanced_graph_feature, 1,
                                            current_index.repeat(1, 1, embedding_dim))
        prev_option_feature = current_node_feature + self.option_embedding1(prev_option).squeeze(1)  # (batch_size, 1, embedding_dim)
        termination_logits = self.termination_head(prev_option_feature).squeeze(-1)
        termination_prob = torch.sigmoid(termination_logits)
        is_termination = (termination_prob > 0.5).squeeze(-1)  # (batch_size,)
        is_termination = is_termination.view(-1, 1, 1)
        current_option = prev_option  # (batch_size, 1, 1)
        current_option = torch.where(is_termination, 1 - prev_option, prev_option)
        current_node_feature, enhanced_current_node_feature = self.decode_state(
            enhanced_graph_feature, current_index, node_padding_mask,
            current_option)

        logp = self.output_policy(current_node_feature, enhanced_current_node_feature,
                                  enhanced_graph_feature, current_edge, edge_padding_mask)
        return logp, termination_logits, current_option
...
class QNet(nn.Module):
    def __init__(self, node_dim, embedding_dim):
        ...
        self.option_embedding = nn.Embedding(2, embedding_dim)

        self.q_values_layer = nn.Linear(embedding_dim * 4, 1)

    def encode_graph(self, node_inputs, node_padding_mask, edge_mask):
        node_feature = self.initial_embedding(node_inputs)
        enhanced_node_feature = self.update_graph_encoder(src=node_feature,
                                             key_padding_mask=node_padding_mask,
                                             attn_mask=edge_mask)
    ...
    def decode_state(self, enhanced_node_feature, current_index, node_padding_mask,
        ...
        current_node_feature = torch.gather(enhanced_node_feature, 1,
                                            current_index.repeat(1, 1, embedding_dim))
        current_node_feature = current_node_feature + self.option_embedding(prev_option).squeeze(1)  # (batch_size, 1, embedding_dim)
        enhanced_current_node_feature, _ = self.decoder(current_node_feature,
    ...
    def forward(self, node_inputs, node_padding_mask, edge_mask, current_index, current_edge, 
        ...
        current_node_feature, enhanced_current_node_feature = self.decode_state(enhanced_node_feature, current_index, node_padding_mask, prev_option)
        q_values = self.output_q(current_node_feature, enhanced_current_node_feature, enhanced_node_feature, current_edge, 
